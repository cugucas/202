...
public class OkHttpClientUtils {
    ...
    private OkHttpProps okHttpProps = new OkHttpProps();

//    private static File file = new File("F:/test");
//    private static final long cacheSize = 1024 * 1024 * 20;// 缓存文件最大限制大小20M
//    private static String cacheDirectory = "F:/okHttpCache";
//    private static Cache cache = new Cache(new File(cacheDirectory), cacheSize);  //

    public class OkHttpProps{
        private  int readTimeout = 600000; //读取超时时间毫秒
        private  int writeTimeout = 600000; //写数据超时时间毫秒
        private  int connectTimeout = 600000; //连接超时时间
            ...
            synchronized (OkHttpClientUtils.class) {
                if (client == null) {
                    client = new OkHttpClient().newBuilder()
                            .readTimeout(okHttpProps.getReadTimeout(), TimeUnit.MILLISECONDS)
                            .writeTimeout(okHttpProps.getWriteTimeout(), TimeUnit.MILLISECONDS)
                            .connectTimeout(okHttpProps.getConnectTimeout(), TimeUnit.MILLISECONDS)
                            .connectionPool(new ConnectionPool(okHttpProps.getMaxIdleConnection(), okHttpProps.getKeepAliveDuration(), TimeUnit.MINUTES))
